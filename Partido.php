...
class Partido
{
    ...
    {
        $coleccionEquipos = []; //Si no hay empate se devuelve 1 equipo en el arreglo
        if ($this->getCantGolesE1() > $this->getCantGolesE2()) {
            $coleccionEquipos[] = $this->getObjEquipo1();
        } elseif ($this->getCantGolesE2() > $this->getCantGolesE1()) {
            $coleccionEquipos[] = $this->getObjEquipo2();
        } else {
            $coleccionEquipos = [
                $this->getObjEquipo1(),
                $this->getObjEquipo2(),
     ...
     * retorna el valor obtenido por el coeficiente base, multiplicado por la
     * cantidad de goles y la cantidad de jugadores. Redefinir dicho método según
     * corresponda.
     */
    public function coeficientePartido()
    {
        $valor =
            $this->getCoefBase() *
            ($this->getCantGolesE1() + $this->getCantGolesE2()) *
            ($this->getObjEquipo1()->getCantJugadores() +
                $this->getObjEquipo2()->getCantJugadores());
